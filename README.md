...
| Event | When | Fields |
|-------|------|--------|
| `DialogueStartEvent` | Dialogue opened | `player`, `entity`, `dialogueId` |
| `DialogueChoiceEvent` | Option selected OR `fire_event` action fired | `player`, `npc`, `dialogueId`, `nodeId`, `source` (`"option"`/`"action"`), `tag` |
| `DialogueEndEvent` | Dialogue completed (end node reached) | `player`, `entity`, `dialogueId` |
...
Plot: the player meets a blacksmith → he asks for 10 iron → hands over a key → a sage appears in front of the player → the sage gives a "kill 10 zombies" quest with a 3-day deadline → return to the sage for a reward.

### 35.1 `dialogues/blacksmith.json` (per-player)

```json
{
  "target": { "name": "Blacksmith", "tag": "blacksmith", "entity_type": "interactentity:custom_npc" },
  "display_name": "&6Blacksmith Borgin",
  "scope": "per_player",
  "entry": "intro",
  "avatar": "blacksmith",
  "faction": "Blacksmiths' Guild",
  "reputation_id": "blacksmiths",
  "visual": {
    "model": "interactentity:geo/custom_npc_default.geo.json",
    "texture": "blacksmith",
    "scale": 1.0
  ...
  "summon": {
    "entity": "minecraft:villager",
    "custom_name": "Blacksmith",
    "tags": ["blacksmith"],
    "trigger": { "type": "on_join", "delay": 60 }
  },
  "nodes": {
    "intro": {
      "text": "&fGreetings, traveler. I'm Borgin the smith.",
      "options": [
        { "text": "What are you working on?", "next": "quest_offer" },
        { "text": "Good luck.",               "next": null }
      ]
    },
    "quest_offer": {
      "text": "Bring me 10 iron ingots and I'll forge you a key.",
      "options": [
        {
          "text": "(Accept)",
          "next": "accept",
          "actions": [
            { "type": "start_quest", "quest": {
                "id": "blacksmith_iron",
                "title": "Iron for Borgin",
                "description": "Bring the smith 10 iron ingots.",
                "objectives": ["Bring 10 iron"],
                "required_item": { "id": "minecraft:iron_ingot", "count": 10 }
            }}
          ]
        },
        { "text": "(Decline)", "next": null }
      ]
    },
    "accept": {
    ...
    "give_key": {
      "text": "Excellent! Here's the key.",
      "actions": [
        { "type": "remove_item", "item": "minecraft:iron_ingot", "count": 10 },
        { "type": "give_item", "item": "minecraft:tripwire_hook" },
        { "type": "complete_quest", "quest_id": "blacksmith_iron" },
        { "type": "add_reputation", "id": "blacksmiths", "value": 20 },
        { "type": "notify_npc", "dialogue_id": "old_sage" },
        { "type": "set_var", "name": "got_key", "value": "1" }
      ],
      "next": null
    }
  },
  "on_revisit": {
    ...
    "conditions": [
      {
        "condition": { "type": "quest_status", "quest_id": "blacksmith_iron", "status": "active" },
        "start_node": "check_iron"
      },
      {
        "condition": { "type": "if_var", "name": "got_key", "value": "1", "op": "eq" },
        "text": "&7Found the sage yet?"
      }
    ]
  }
}
...
### 35.2 `dialogues/old_sage.json` (per-player)

```json
{
  "target": { "name": "Sage", "tag": "sage", "entity_type": "interactentity:custom_npc" },
  "display_name": "&dThe Sage",
  "scope": "per_player",
  "entry": "intro",
  "avatar": "sage",
  "visual": { "texture": "sage", "scale": 1.05 },
  "summon": {
    "entity": "interactentity:custom_npc",
    "custom_name": "Sage",
    "tags": ["sage"],
    "trigger": { "type": "after_dialogue", "dialogue_id": "blacksmith", "delay": 100 },
    "spawn_position": "front_of_player"
  },
  "nodes": {
    "intro": {
      "text": "You came with the key. I was expecting you.",
      "actions": [{ "type": "play_emote", "emote": "bow", "duration_ticks": 40 }],
      "next": "task"
    },
    "task": {
      "text": "Kill 10 zombies nearby. The village is in danger.",
      "options": [
        {
          "text": "(Accept)",
          "next": "accepted",
          "actions": [
            { "type": "start_quest", "quest": {
                "id": "sage_zombies",
                "title": "Cleanup",
                "description": "The sage asks you to kill 10 zombies.",
                "objectives": ["Kill zombies"],
                "required_kills": { "entity": "minecraft:zombie", "count": 10, "objective": 0 },
                "deadline": { "type": "game_days", "value": 3 }
            }},
            { "type": "fire_event", "tag": "sage_quest_started" }
          ]
        }
      ]
    },
    "accepted": {
      "text": "Go now. Time is running out.",
      "next": null
    },
    "reward": {
      "text": "The village is in your debt.",
      "actions": [
        { "type": "complete_quest", "quest_id": "sage_zombies" },
        { "type": "give_item", "item": "minecraft:emerald", "count": 5 },
        { "type": "add_reputation", "id": "blacksmiths", "value": 30 },
        { "type": "play_emote", "emote": "handshake" }
      ],
      "next": null
    }
  },
  "on_revisit": {
    "conditions": [
      {
        "condition": { "type": "killed_mob", "entity": "minecraft:zombie", "count": 10 },
        "start_node": "reward"
      },
      {
        "condition": { "type": "quest_status", "quest_id": "sage_zombies", "status": "active" },
        "text": "&7Come back when it's done."
      }
    ],
    "default": "&7*gazes into the distance*"
  }
}
```
...
```
<world>/interactentity/dialogues/blacksmith.json
<world>/interactentity/dialogues/old_sage.json
<world>/interactentity/skins/blacksmith.png   ← 64x64
<world>/interactentity/skins/sage.png         ← 64x64
```

After starting: `/dialogue reload`, then find or wait for the blacksmith to spawn. Talk → bring iron → the sage appears → kill 10 zombies → come back to the sage.

### 35.4 KubeJS hook on the event

```js
// kubejs/server_scripts/sage_quest.js

ForgeEvents.onEvent('net.ashpapi.interactentity.api.DialogueChoiceEvent', event => {
  if (event.tag === 'sage_quest_started') {
    event.player.tell('§5The sage looks at you with piercing eyes...')
    event.player.runCommandSilent('weather thunder 600')
  }
})

ForgeEvents.onEvent('net.ashpapi.interactentity.api.QuestCompleteEvent', event => {
  if (event.questId === 'sage_zombies') {
    event.player.runCommandSilent('xp add @s 100 levels')
  }
})
```
...
| Event | Когда | Поля |
|-------|-------|------|
| `DialogueStartEvent` | Диалог открыт | `player`, `entity`, `dialogueId` |
| `DialogueChoiceEvent` | Выбран ответ ИЛИ сработал `fire_event` | `player`, `npc`, `dialogueId`, `nodeId`, `source` (`"option"`/`"action"`), `tag` |
| `DialogueEndEvent` | Диалог завершён (end-нод) | `player`, `entity`, `dialogueId` |
...
Сюжет: игрок встречает кузнеца → тот просит принести 10 железа → выдаёт ключ → перед игроком появляется старец → старец даёт квест убить 10 зомби с дедлайном 3 игровых дня → возврат к старцу за наградой.

### 35.1 `dialogues/blacksmith.json` (per-player)

```json
{
  "target": { "name": "Кузнец", "tag": "blacksmith", "entity_type": "interactentity:custom_npc" },
  "display_name": "&6Кузнец Боргин",
  "scope": "per_player",
  "entry": "intro",
  "avatar": "blacksmith",
  "faction": "Гильдия Кузнецов",
  "reputation_id": "blacksmiths",
  "visual": {
    "model": "interactentity:geo/custom_npc_default.geo.json",
    "texture": "blacksmith",
  ...
  "nodes": {
    "intro": {
      "text": "&fПривет, путник. Я кузнец Боргин.",
      "options": [
        { "text": "Чем занят?", "next": "quest_offer" },
        { "text": "Удачи в работе.", "next": null }
      ]
    },
    "quest_offer": {
      "text": "Принеси мне 10 железа — выкую тебе ключ.",
      "options": [
        {
          "text": "(Согласиться)",
          "next": "accept",
          "actions": [
            { "type": "start_quest", "quest": {
                "id": "blacksmith_iron",
                "title": "Железо для Боргина",
                "description": "Принеси кузнецу 10 железных слитков.",
                "objectives": ["Принести 10 железа"],
                "required_item": { "id": "minecraft:iron_ingot", "count": 10 }
            }}
          ]
        },
        { "text": "(Отказаться)", "next": null }
      ]
    },
    "accept": {
      "text": "Жду тебя.",
...
```json
{
  "target": { "name": "Старец", "tag": "sage", "entity_type": "interactentity:custom_npc" },
  "display_name": "&dСтарец",
  "scope": "per_player",
  "entry": "intro",
  "avatar": "sage",
  "visual": { "texture": "sage", "scale": 1.05 },
  ...
  "nodes": {
    "intro": {
      "text": "Ты пришёл с ключом. Я ждал тебя.",
      "actions": [{ "type": "play_emote", "emote": "bow", "duration_ticks": 40 }],
      "next": "task"
    },
    "task": {
      "text": "Убей 10 зомби в окрестностях. Деревня в опасности.",
      "options": [
        {
          "text": "(Принять)",
          "next": "accepted",
          "actions": [
            { "type": "start_quest", "quest": {
                "id": "sage_zombies",
                "title": "Зачистка",
                "description": "Старец просит убить 10 зомби.",
                "objectives": ["Убить зомби"],
                "required_kills": { "entity": "minecraft:zombie", "count": 10, "objective": 0 },
                "deadline": { "type": "game_days", "value": 3 }
            }},
    ...
    "accepted": {
      "text": "Иди же. Время на исходе.",
      "next": null
    },
    "reward": {
      "text": "Деревня в долгу перед тобой.",
      "actions": [
        { "type": "complete_quest", "quest_id": "sage_zombies" },
        { "type": "give_item", "item": "minecraft:emerald", "count": 5 },
        { "type": "add_reputation", "id": "blacksmiths", "value": 30 },
  ...
  "on_revisit": {
    "conditions": [
      ...
      {
        "condition": { "type": "quest_status", "quest_id": "sage_zombies", "status": "active" },
        "text": "&7Возвращайся когда зачистишь."
      }
    ],
    "default": "&7*смотрит вдаль*"
  }
}
```
...
После запуска: `/dialogue reload`, потом найди или подожди спавна кузнеца. Поговори → принеси железо → старец появится перед тобой → убей 10 зомби → вернись к старцу.
...
ForgeEvents.onEvent('net.ashpapi.interactentity.api.DialogueChoiceEvent', event => {
  if (event.tag === 'sage_quest_started') {
    event.player.tell('§5Старец смотрит на тебя проницательно...')
    event.player.runCommandSilent('weather thunder 600')
  }
})
